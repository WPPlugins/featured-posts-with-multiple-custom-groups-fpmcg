...
class Custom_Featured_Post_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$image_width   			= empty($instance['image_width']) ? '65' : $instance['image_width'];
		$image_height   		= empty($instance['image_height']) ? '65' : $instance['image_height'];
		...
		if (count($instance['post_cat_type']) > 0) {
			foreach($instance['post_cat_type'] as $pcattype) {
				...
				$query = "
					...
					AND ( m1.meta_key = 'wafysol_fpmcg_pctfp' AND find_in_set( ".$pcattype.", m1.meta_value ) <>0 )
				...
				if (count($posts_list) > 0) { 
					...
					foreach ($posts_list as $posts) {
						$images = get_children( array( 'post_parent' => $posts->ID, 'post_type' => 'attachment', 'post_mime_type' => 'image', 'orderby' => 'menu_order', 'order' => 'ASC', 'numberposts' => 999 ) );
						...
						if ( $featured_image_id != ""  && $displaypostimage == '1' ) :
                            ...
                            $image_img_tag = wp_get_attachment_image( $featured_image_id, array($image_width,$image_height) );
                        ...
                        <?php
							...
							if ( $images && $displaypostimage == '1' ) :
								...
								$image_img_tag = wp_get_attachment_image( $image->ID, array($image_width,$image_height) );
								echo $before_image; ?>
	...
	function update( $new_instance, $old_instance ) {   
        ...
        if ( current_user_can('unfiltered_html') ) {
			...
			$instance['image_width']            = empty( $new_instance['image_width'] ) ? '65' : $new_instance['image_width'];
			$instance['image_height']           = empty( $new_instance['image_height'] ) ? '65' : $new_instance['image_height'];
			...
			$instance['image_width']            = empty( $new_instance['image_width'] ) ? '65' : stripslashes( wp_filter_post_kses( addslashes($new_instance['image_width']) ) );
			$instance['image_height']           = empty( $new_instance['image_height'] ) ? '65' : stripslashes( wp_filter_post_kses( addslashes($new_instance['image_height']) ) );
	...
	function form( $instance ) {
		...
		$image_width 				= esc_textarea($instance['image_width']);
		$image_height 				= esc_textarea($instance['image_height']);
		...
		echo '<p> <label for="'. $this->get_field_id('image_width') .'">'. _e( 'Post Image Width:', FB_RSSI_TEXTDOMAIN ) .' <input class="widefat" id="'. $this->get_field_id('image_width').'" name="'. $this->get_field_name('image_width').'" type="text" value="'. $image_width .'" /></label>
                      </p>';
					  
		echo '<p> <label for="'. $this->get_field_id('image_height') .'">'. _e( 'Post Image Height:', FB_RSSI_TEXTDOMAIN ) .' <input class="widefat" id="'. $this->get_field_id('image_height').'" name="'. $this->get_field_name('image_height').'" type="text" value="'. $image_height .'" /></label>
                      </p>';
